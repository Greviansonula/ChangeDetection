# -*- coding: utf-8 -*-
"""
Created on Tue Aug  9 17:23:50 2022

@author: H
"""
# 代码来源 https://github.com/ZhengJianwei2/ICIF-Net/blob/main/models/ICIFNet.py

import torch
import torch.nn as nn
from A10_1ResNet import resnet18
from A10_2Pvtv2 import pvt_v2_b1
...
class ICIFNet(nn.Module):
    def __init__(self,in_ch, num_classes, drop_rate=0.2, normal_init=True, pretrained=False):
        super(ICIFNet, self).__init__()

        self.show_Feature_Maps = False
        self.resnet = resnet18()
        # if pretrained:
        self.resnet.load_state_dict(torch.load('./A10_3resnet18-5c106cde.pth'))
        # self.resnet.fc = nn.Identity()
        self.resnet.layer4 = nn.Identity()

        self.backbone = pvt_v2_b1()  # [64, 128, 320, 512]
        path = './A10_4pvt_v2_b1.pth'
...
if __name__ == "__main__":
    ...
    model = ICIFNet(3,2, pretrained=True).to(device)
    input_A = torch.randn(1, 3, 256, 256).to(device)
    input_B = torch.randn(1, 3, 256, 256).to(device)
    flops, params = profile(model, inputs=(input_A,input_B ))
